    ...
    with tf.gfile.GFile(input_file, "r") as reader:
      while True:
        ...
        if input_tokens and target_tokens:
          all_documents[-1].append((input_tokens,target_tokens))
...
def create_csc_masked_lm_predictions(tokens_input,tokens_target, masked_lm_prob,
    ...
    for idx,(input_token,target_token) in enumerate(zip(tokens_input,tokens_target)):
      if input_token == "[CLS]" or input_token == "[SEP]":
        continue
...
def create_instances_from_document(
  ...
  (input_tokens,target_tokens) = all_documents[document_index][0] 
  ...
  segment_ids = []
  tokens.append("[CLS]")
  tokens_correct.append("[CLS]")
  segment_ids.append(0)
  
  for input_token,target_token in zip(input_tokens,target_tokens):
    tokens.append(input_token)
    tokens_correct.append(target_token)
    segment_ids.append(0)

  tokens.append("[SEP]")
  tokens_correct.append("[SEP]")
  segment_ids.append(0)
  ...
  (tokens, masked_lm_positions,
    masked_lm_labels) = create_csc_masked_lm_predictions(
        tokens, tokens_correct,masked_lm_prob, max_predictions_per_seq, vocab_words, rng)
